...
SKIP_PATTERNS = {
    '*.min.js', '*.min.ts', '*.min.css',
    # config files — tool configs, not source code
    'eslint.config.*', '.eslintrc.*',
    'prettier.config.*', '.prettierrc.*',
    'jest.config.*', 'vitest.config.*',
    'esbuild.config.*', 'webpack.config.*', 'vite.config.*',
    'rollup.config.*', 'tsconfig.*', 'jsconfig.*',
    'babel.config.*', '.babelrc.*',
    'postcss.config.*', 'tailwind.config.*',
    'lint-staged.config.*', 'release.config.*',
...
def _pass1_collect_approved_folders(path: str, ignore_folders: set, ignore_substrings: set = set()) -> set:
    """
    ...
    def should_ignore(folder_name: str) -> bool:
        name_lower = folder_name.lower()
        if name_lower in ignore_folders:
            return True
        if any(sub in name_lower for sub in ignore_substrings):
            return True
        return False
...
def scan_directory(path="."):
    ...
    # substring patterns — any folder name CONTAINING these words gets ignored
    ignore_substrings = {'test', 'mock', 'fixture', 'snapshot', 'stub'}

    # --- PASS 1: collect approved folders (no file reads) ---
    approved_folders = _pass1_collect_approved_folders(path, ignore_folders, ignore_substrings)
